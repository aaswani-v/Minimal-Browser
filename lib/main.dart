...
class BrowserTab {
  ...
  BrowserTab({required this.id, this.url = "about:blank", this.title = "New Tab"});
}
...
class Note {
  ...
  Map<String, dynamic> toJson() => {'content': content, 'timestamp': timestamp.toIso8601String()};
  static Note fromJson(Map<String, dynamic> json) => Note(content: json['content'], timestamp: DateTime.parse(json['timestamp']));
}
...
class TodoItem {
  ...
  static TodoItem fromJson(Map<String, dynamic> json) => TodoItem(task: json['task'], isDone: json['isDone']);
}
...
class QuickAILink {
  ...
  static QuickAILink fromJson(Map<String, dynamic> json) => QuickAILink(name: json['name'], url: json['url']);
}
...
class _BrowserScreenState extends State<BrowserScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      final RenderBox? renderBox = _navBarKey.currentContext?.findRenderObject() as RenderBox?;
      if (renderBox != null) {
        setState(() {
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      ...
      _notes = (jsonDecode(notesString) as List).map((n) => Note.fromJson(n)).toList();

      final todosString = prefs.getString('todos') ?? '[]';
      _todos = (jsonDecode(todosString) as List).map((t) => TodoItem.fromJson(t)).toList();
      ...
      if (aiLinksString != null) {
        _quickAILinks = (jsonDecode(aiLinksString) as List).map((l) => QuickAILink.fromJson(l)).toList();
      }
    });
  }
  ...
  Future<void> _saveData() async {
    ...
    await prefs.setString('quick_ai_links', jsonEncode(_quickAILinks.map((l) => l.toJson()).toList()));
  }
  ...
  void _handleHomeScroll() {
    if (_homeScrollController.position.userScrollDirection == ScrollDirection.reverse) {
      if (_isNavBarVisible) setState(() => _isNavBarVisible = false);
    }
    if (_homeScrollController.position.userScrollDirection == ScrollDirection.forward) {
  ...
  InAppWebViewController? get _activeWebViewController => _webViewControllers[_activeTab.id];


  void _addTab({bool isFirstTab = false, String url = "about:blank", String title = "New Tab", bool isMusicTab = false}) {
    setState(() {
      final newTab = BrowserTab(id: DateTime.now().millisecondsSinceEpoch, url: url, title: title);
  ...
  void _loadUrl(String url) {
    ...
    if (_activeWebViewController != null) {
      _activeWebViewController!.loadUrl(urlRequest: URLRequest(url: WebUri(url)));
      setState(() {
        _activeTab.url = url;
  ...
  Future<bool> _onWillPop() async {
    ...
    if (_activeWebViewController != null && await _activeWebViewController!.canGoBack()) {
  ...
  void _updateMusicInfo() async {
    ...
    final mediaInfoJson = await musicWebViewController.evaluateJavascript(source: getMediaInfoJs);
    final playButtonAriaLabel = await musicWebViewController.evaluateJavascript(source: "document.getElementById('play-pause-button')?.getAttribute('aria-label')");
    ...
    const getTitleJs = "document.querySelector('ytmusic-player-bar .title.yt-formatted-string')?.innerText";
    const getArtistJs = "document.querySelector('ytmusic-player-bar .byline.yt-formatted-string')?.innerText.split('•')[0].trim()";
    const getThumbnailJs = "document.querySelector('ytmusic-player-bar #song-image img')?.src";

    final titleResult = await musicWebViewController.evaluateJavascript(source: getTitleJs);
    final artistResult = await musicWebViewController.evaluateJavascript(source: getArtistJs);
    final thumbnailResult = await musicWebViewController.evaluateJavascript(source: getThumbnailJs);

    final String? title = (titleResult is String && titleResult.isNotEmpty) ? titleResult : null;
    final String? artist = (artistResult is String && artistResult.isNotEmpty) ? artistResult : null;
    final String? thumbnailUrl = (thumbnailResult is String && thumbnailResult.isNotEmpty) ? thumbnailResult : null;
          ...
          child: Stack(
            children: [
              Container(
                decoration: const BoxDecoration(
                  ...
                  gradient: RadialGradient(
                    center: Alignment(-0.6, -0.5), radius: 0.8,
                    colors: [Color(0x66A0A0A0), Colors.black],
                  ),
                ),
                child: Container(
                  decoration: const BoxDecoration(
                    gradient: RadialGradient(
                      center: Alignment(0.6, 0.5), radius: 0.8,
                      colors: [Color(0x668C8C8C), Colors.transparent],
                    ),
                  ),
                ),
              ),
              AnimatedPadding(
                ...
                padding: EdgeInsets.only(bottom: _isNavBarVisible && _showWebContent ? _navBarHeight : 0),
                child: Stack(
                  children: [
                    Stack(
                        ...
                        return Offstage(
                          ...
                          child: InAppWebView(
                              ...
                              if (url != null && url.toString().startsWith("http")) {
                                ...
                                if (mounted) {
                                  ...
                                  if (tab.isYouTubeMusicTab) {
                                    ...
                                    Future.delayed(const Duration(seconds: 1), _updateMusicInfo);
                                  }
                                }
                              }
                            },
                            ...
                            onProgressChanged: (controller, progress) {
                              if (mounted) {
                                setState(() {
                                  if (index == _activeTabIndex) _progress = progress / 100.0;
                                });
                              }
                            },
                    ...
                    if (!_showWebContent)
                      HomeScreenContent(
                        ...
                        onLoadUrl: _loadUrl,
                        onAddAILink: _showAddAIDialog, // Pass the method to the home screen
                        onOpenMusic: () {
                          _addTab(url: "https://music.youtube.com", title: "YouTube Music", isMusicTab: true);
                        },
                        ...
                        onMusicPrevious: () {
                          final musicTab = _tabs.firstWhere((t) => t.isYouTubeMusicTab, orElse: () => _activeTab);
                          _webViewControllers[musicTab.id]?.evaluateJavascript(source: "document.querySelector('.previous-button').click()");
                          Future.delayed(const Duration(milliseconds: 500), _updateMusicInfo);
                        },
                        onMusicPlayPause: () {
                          final musicTab = _tabs.firstWhere((t) => t.isYouTubeMusicTab, orElse: () => _activeTab);
                          _webViewControllers[musicTab.id]?.evaluateJavascript(source: "document.getElementById('play-pause-button').click()");
                          Future.delayed(const Duration(milliseconds: 500), _updateMusicInfo);
                        },
                        onMusicNext: () {
                          final musicTab = _tabs.firstWhere((t) => t.isYouTubeMusicTab, orElse: () => _activeTab);
                          _webViewControllers[musicTab.id]?.evaluateJavascript(source: "document.querySelector('.next-button').click()");
                          Future.delayed(const Duration(milliseconds: 500), _updateMusicInfo);
                        },
                      ),
                  ],
              ...
              Positioned(
                ...
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 300),
                  curve: Curves.easeInOut,
                  transform: Matrix4.translationValues(0, _isNavBarVisible ? 0 : 200, 0),
                  child: Padding(
                    ...
                    child: FloatingNavBar(
                      ...
                      onNewTab: () => _addTab(),
                      onSearchTap: () => setState(() => _isSearchVisible = true),
                    ),
                  ),
                ),
              ),
  ...
  void _showSearchDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF1E1E1E),
        title: const Text("Search or Type URL", style: TextStyle(color: Colors.white)),
        content: TextField(
          controller: textController,
          autofocus: true,
          style: const TextStyle(color: Colors.white),
  ...
  void _showChatAIDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF1E1E1E),
        title: const Text("Choose AI Tool", style: TextStyle(color: Colors.white)),
        content: SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                leading: Image.network(aiLink.iconUrl, width: 24, height: 24, errorBuilder: (c, e, s) => const Icon(Icons.computer)),
                title: Text(aiLink.name, style: const TextStyle(color: Colors.white)),
                onTap: () {
                  Navigator.of(context).pop();
                  _loadUrl(aiLink.url);
                },
  ...
  void _showQuickNoteDialog() {
    final TextEditingController textController = TextEditingController(text: _notes.isNotEmpty ? _notes.first.content : "");
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF1E1E1E),
...
class GlassWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // MODIFIED: Made the fallback decoration more opaque to hide background content
        : BoxDecoration(
      gradient: LinearGradient(
        colors: [
      ...
      borderRadius: borderRadius,
      border: Border(top: BorderSide(color: Colors.white.withOpacity(0.15), width: 1.0)),
    );
...
class FloatingNavBar extends StatelessWidget {
                  ...
                  child: Row(
                    children: [
                      activeTab.favicon != null
                        ...
                        child: Image.network(
                          activeTab.favicon.toString(),
                          width: 20,
                          height: 20,
                          errorBuilder: (context, error, stackTrace) => const Icon(Icons.search, color: Colors.white54, size: 20),
                        ),
                      )
                      ...
                      Expanded(
                        child: Text(
                          activeTab.url.startsWith("http") ? activeTab.title : "Search or type URL",
  ...
  Widget _buildTab(BuildContext context, BrowserTab tab, {required bool isActive, required VoidCallback onTap, required VoidCallback onClose}) {
    return GestureDetector(
      onTap: onTap,
      onDoubleTap: onClose, // Double press to close tab
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 12.0), // Symmetrical padding
        decoration: BoxDecoration(
          color: isActive ? Colors.white.withOpacity(0.2) : Colors.transparent,
          borderRadius: BorderRadius.circular(20.0),
          border: Border.all(color: Colors.white.withOpacity(isActive ? 0.3 : 0.1)),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            if (tab.favicon != null) ...[
              Image.network(
                ...
                width: 18, height: 18,
                errorBuilder: (context, error, stackTrace) => const Icon(Icons.public, size: 18, color: Colors.white70),
              ),
              const SizedBox(width: 8),
            ] else ... [
            ...
            // The close button Icon is removed from here
          ],
        ),
      ),
    );
            ...
            GlassWidget(
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("Favourites", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 16)),
                    const SizedBox(height: 16),
                    GridView.count(
                      crossAxisCount: 4,
                      ...
                      children: [
                        ...favourites.map((fav) => _FavouriteButton(favourite: fav, onTap: () => onLoadUrl(fav.url))),
...
class _NotesWidgetState extends State<_NotesWidget> {
  ...
  void initState() {
    super.initState();
    _noteController = TextEditingController(text: widget.notes.isNotEmpty ? widget.notes.first.content : "");
  }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Quick Notes", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 16)),
          const SizedBox(height: 8),
          Expanded(
            child: TextField(
              controller: _noteController,
              onChanged: widget.onNoteChanged, // Use onChanged for safer state updates.
              expands: true,
              maxLines: null,
...
class _TodosWidget extends StatelessWidget {
  ...
  const _TodosWidget({required this.todos, required this.onAddTodoTap, required this.onToggleTodo});
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              const Text("To-Do List", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 16)),
              IconButton(
          ...
          if (songThumbnailUrl != null)
            ClipRRect(
              ...
              child: Image.network(
                songThumbnailUrl!,
                fit: BoxFit.cover,
                errorBuilder: (context, error, stack) => const Icon(Icons.music_note, size: 56, color: Colors.white30),
              ),
            ),
          Container(
            decoration: BoxDecoration(
              ...
              gradient: LinearGradient(
                colors: [
                  Colors.black.withOpacity(songThumbnailUrl != null ? 0.6 : 0.0),
                  Colors.transparent,
            ...
            child: Column(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(songTitle, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold), maxLines: 2, overflow: TextOverflow.ellipsis,),
                    const SizedBox(height: 2),
                    Text(songArtist, style: const TextStyle(color: Colors.white70, fontSize: 12), maxLines: 1, overflow: TextOverflow.ellipsis,),
                  ],
                ),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: [
                    IconButton(icon: const Icon(Icons.skip_previous, color: Colors.white70), onPressed: onPrevious, padding: EdgeInsets.zero, visualDensity: VisualDensity.compact),
                    IconButton(icon: Icon(isPlaying ? Icons.pause_circle_filled : Icons.play_circle_filled, color: Colors.white, size: 40), onPressed: onPlayPause, padding: EdgeInsets.zero, visualDensity: VisualDensity.compact),
                    IconButton(icon: const Icon(Icons.skip_next, color: Colors.white70), onPressed: onNext, padding: EdgeInsets.zero, visualDensity: VisualDensity.compact),
                  ],
                ),
              ],
            ),
...
class _QuickAIAccessWidget extends StatelessWidget {
  ...
  const _QuickAIAccessWidget({required this.quickAILinks, required this.onLoadUrl, required this.onAddAILink});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Quick AI Access", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 16)),
          const SizedBox(height: 16),
          Expanded(
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _QuickAIButton(link: link, onTap: () => onLoadUrl(link.url));
              },
            ),
          ),
        ],
        ...
        child: Column(
          ...
          children: [
            Image.network(
              ...
              width: 24,
              height: 24,
              errorBuilder: (context, error, stackTrace) => const Icon(Icons.computer, size: 24, color: Colors.white70),
            ),
            const SizedBox(height: 8),
            Flexible(
              child: Text(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text("Add Tool", style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 12)),
          ],
        ),
      ),
    );
          ...
          ClipOval(
            child: Image.network(
              ...
              errorBuilder: (context, error, stackTrace) =>
                  Text(favourite!.title, style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
            ),
          )
            ...
            child: Column(
              children: [
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: [
                    _SearchActionButton(icon: Icons.summarize, label: "Summarize", onTap: () {}),
                    _SearchActionButton(icon: Icons.chat_bubble_outline, label: "ChatAI", onTap: onShowChatAIDialog),
                    _SearchActionButton(icon: Icons.send_outlined, label: "Send", onTap: onShare),
                    _SearchActionButton(icon: Icons.description_outlined, label: "Quick Note", onTap: onQuickNote),
                  ],
                ),
                const SizedBox(height: 24),
                // Link List
...
class _SearchActionButton extends StatelessWidget {
  ...
  const _SearchActionButton({required this.icon, required this.label, required this.onTap});
...
class _SearchListItem extends StatelessWidget {
  ...
  const _SearchListItem({required this.title, required this.subtitle, required this.iconUrl, required this.onTap});
            ...
            child: Row(
              children: [
                Image.network(
                  ...
                  width: 24,
                  height: 24,
                  errorBuilder: (context, error, stackTrace) => const Icon(Icons.public, size: 24, color: Colors.white70),
                ),
                const SizedBox(width: 16),
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
